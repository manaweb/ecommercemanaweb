...
CLASS ISC_PRODUCTDETAILS_PANEL extends PANEL
{
	...
	private function SetShippingCost()
	{
		...
		if ($this->productClass->GetFixedShippingCost() != 0) {
			...
			$GLOBALS['ShippingPrice'] = '<img src="'.$GLOBALS['ShopPath'].'/img/fretegratis.gif"><br>';
		}
	...
	if(!isset($GLOBALS['ISC_CLASS_PRODUCT']) || !$GLOBALS['ISC_CLASS_PRODUCT']->hasVideos()) {
		...
		$GLOBALS["TabelaParcela1"]='<br>';
		$GLOBALS["TabelaParcela1"].='<table width="230px" border="0" cellspacing="0"  style="font-size:11px;" cellpadding="0">';
		for($zuzuba=1;$zuzuba<=$numero_parcela;$zuzuba++){
		 	$index = $c % 2;
		    $c++;
		    $cor = $cores[$index]; 
			$GLOBALS["TabelaParcela1"].=Criaparcelas($finalPrice,$zuzuba,$cor);
		}
		$GLOBALS["TabelaParcela1"].='</table>';
		

		///////////		
		print MostraParcela($finalPrice);
	...
	private function SetPurchasingOptions()
	{
		...
		if($productInStock == true) {
			...
			$GLOBALS['SNIPPETS']['SideAddItemSoldOut'] = $output.'<a href="#mostrar" id="meavise" class="fancybox"><button style="width: 200px;height: 30px; border: none;background-color: purple;color: white;cursor: inherit;">Me avise quando chegar!</button></a>';
	...
	private function SetProductImages()
	{
		...
		$GLOBALS['TinyImageClickJavascript'] = "showProductImage('".GetConfig('ShopPath')."/productimage.php', ".$GLOBALS['ProductId'].");";
